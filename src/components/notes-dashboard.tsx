...
export function NotesDashboard({ notes, loadError, showUpload }: { notes: NoteSummary[]; loadError: boolean; showUpload: boolean }) {
  ...
  const [uploadOpen, setUploadOpen] = useState(showUpload);
  const [query, setQuery] = useState('');
  const upload = uploadOpen || showUpload || notes.length === 0;
  ...
  return <>
    ...
    <main id="main-content" className="workspace">
      <div className="workspace-heading"><div><h1>Your notes</h1><p>{notes.length ? 'A little knowledge, ready to go further.' : 'Your next idea deserves a good home.'}</p></div>
        {notes.length > 0 && <button className={`button ${upload ? '' : 'primary'}`} onClick={() => { setUploadOpen(!upload); if (showUpload) router.replace('/dashboard'); }}>{upload ? <X size={16} /> : <Plus size={16} />}{upload ? 'Close upload' : 'Upload note'}</button>}
      </div>
